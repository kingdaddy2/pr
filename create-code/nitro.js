...
async function nitrogenerator(usernameAuthor, avatarAuthor, username, thank, avatar, channel, authorID) {
  ...
  let n = formatAMPM(new Date(Date.now() - 6e4)),
    o = await fs.readFileSync(`./html/testingboost.html`, "utf8");
  (datatosend = o),
...
if(!json.time) return;
if(!e.content.startsWith(`${json.prefix}boost`) && !e.content.startsWith(`${json.prefix}create-boost`)) return;
...
if(!user) return await nitrogenerator(msg.author.username, msg.author.avatarURL, argss[0].replace(json.prefix + 'boost', ''), argss[1], msg.author.avatarURL, msg.channel.id, msg.author.id)

await nitrogenerator(msg.author.username, msg.author.avatarURL, argss[0].replace(json.prefix + 'boost', ''), argss[1], user.avatarURL, msg.channel.id, msg.author.id)
}
  
 


if(commandName === "create-boost"){
